...
start_L = iterations;                      %Start time of leader to move
...
Rk = [3,3,3]; %Radius of obstacles 
yk(1,:,:) = repmat([0 0], 1, 1, size(yk, 3)); %Initialize the center of obstacles
yk(2,:,:) = repmat([0 0], 1, 1, size(yk, 3)); %...center
yk(3,:,:) = repmat([0 0], 1, 1, size(yk, 3)); %...center
q_hat = zeros(Na*Nb , 2 , iterations);
p_hat = zeros(Na*Nb , 2 , iterations);
d_obs = 0; % The distance between agents and the surface of obstacles (d')
...
%%%
d = sin(pi/10)*d_L*2; % Equation   Desired Distance Between Agents
...
%             set(Alpha_Edge(end), 'HandleVisibility', 'off'); 
        else
            Alpha_Edge(end).Color = 'm';

            if ~legend_added  % if legend is not added yet
                edge_Alpha = j+i-2;
                legend_added = true;  % set flag to true
            end
%             set(Alpha_Edge(end), 'HandleVisibility', 'off'); 
        end
    end
...
% str_a = string(1:Na);
% txt = textscatter(0.3+qa(:,1,Tf),0.3+qa(:,2,Tf),str_a);
circles = [];
for i = 1:numel(d_L)
    circles = [circles plot(qg(1,1,Tf) + d_L(i)*circle_x , qg(1,2,Tf) + d_L(i)*circle_y , 'k--','LineWidth',0.25)];
end
% legend([Alpha_Edge(edge_Alpha), circles, Leaders , Agents], {'Edge ($\mathcal{E}_\alpha$)',...
%     'Desired Radius ($d^{\prime\prime}$)' , 'Leader', 'Agent'},'interpreter','latex',...
...
axis equal
print('Irregular_1','-depsc2','-r600')
saveas(P5, 'Irregular_1.png')
% grid on 
% grid minor
...
%     circles = [];
%     circles2= [];
        print('irregular_final','-depsc2','-r600')
    end
    drawnow
